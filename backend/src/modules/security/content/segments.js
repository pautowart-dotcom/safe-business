...
const SEGMENTS = [
  ...
  },
  {
    ...
    hasNicheStep: true,
    // Груминг (12.09.2026) — первая ниша нового сегмента, узкий старт по
    // тому же принципу, что клининг/общепит/бытовые услуги. Структурно
    // ближе к бьюти-нишам (ГК РФ гл.39), чем к бытовому подряду ателье/
    // обуви/химчистки — предмет услуги не вещь, а живое существо, см.
  ...
  {
    key: 'auto',
    label: 'Услуги для автомобилей',
    hasNicheStep: true,
    // Автомойка (14.09.2026) — первая ниша нового сегмента, узкий старт по
    // тому же принципу, что pets/food. Структурно ближе к бьюти-нишам
    // (ГК РФ гл.39, возмездное оказание услуг) — предмет услуги
    // (автомобиль) временно передаётся и возвращается в исходном виде, не
    // видоизменяется как при бытовом подряде. Ключевая специфика —
    // требования к очистке сточных вод (нефтеуловитель), см. комментарий в
    ...
    niches: [
      ...
      // auto_service (15.09.2026) — третья ниша сегмента, ремонт и ТО, тот
      // же правовой каркас плюс собственный профильный акт (ПП №780 вместо
      // утратившего силу ПП №290), см. комментарий в шапке
      // violations/auto-service.js.
      { key: 'auto_service', label: 'Автосервис (ремонт и ТО)', paidAudit: true },
    ],
  },
  {
    key: 'retail',
  ...
  },
  {
    ...
    // paidAudit: true с 10.09.2026 — блок 2 (санитария) дозаполнен по
    // первоисточнику СП 2.3/2.4.4282-26 (действует с 01.09.2026), см.
