...
const SEGMENTS = [
  ...
  },
  {
    ...
    niches: [
      ...
      // facade_cleaning (15.09.2026) — вторая ниша сегмента. Изначально
      // планировался спецклининг/биоклининг — сознательно заменён в
      // процессе разведки на другую нишу из-за находки про новую лицензию
  ...
  },
  {
    ...
    niches: [
      ...
      // device_repair (15.09.2026) — седьмая ниша сегмента, тот же правовой
      // каркас (ПП №1514, подтверждённый как применимый и к телефонам/
      // компьютерам), но с реально новой специфичной для устройств
      // спецификой — доступ к личным данным клиента, см. комментарий в
      // шапке violations/device-repair.js.
      { key: 'device_repair', label: 'Ремонт компьютеров и телефонов', paidAudit: true },
    ],
  },
  {
    key: 'pets',
    ...
    niches: [
      ...
      // dog_training (15.09.2026) — третья ниша сегмента, реальная,
      // подтверждённая первоисточником специфика — ФЗ-498 про потенциально
      // опасные породы, см. комментарий в шапке violations/dog-training.js.
      { key: 'dog_training', label: 'Дрессировка собак', paidAudit: true },
    ],
  },
  {
    key: 'auto',
  ...
  },
  {
    ...
    hasNicheStep: true,
    // kids_club (15.09.2026) — первая ниша нового сегмента, узкий старт по
    // тому же принципу, что pets/auto/food. Сознательно построена для
    // модели "присмотр, уход и досуг" БЕЗ лицензии на образовательную
    // деятельность — лицензированное доп.образование детей качественно
    // тяжелее (СЭЗ до лицензии, другой контур регулирования), сознательно
    // не взято, аналогично тому, как стоматология заблокирована из-за
    // медицинской лицензии, см. комментарий в начале violations/kids-club.js.
    niches: [
      { key: 'kids_club', label: 'Детские клубы и секции', paidAudit: true },
    ],
  },
  {
    key: 'retail',
