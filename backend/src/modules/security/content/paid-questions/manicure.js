...
const QUESTIONS = [
  ...
  {
    // Добавлено 05.09.2026 вместе с MN-405 (violations/manicure.js) — ответ
    // "Да" (индекс 0, баллы 1) также используется как сигнал "документ уже
    // есть" в document-templates/content/templateViolationLinks.js, чтобы
    // не спрашивать о том же самом второй раз при покупке шаблонов.
    code: 'MN-405',
    block: 5,
  ...
  {
    // Добавлено 05.09.2026 вместе с MN-406 (violations/manicure.js).
    code: 'MN-406',
    block: 5,
  ...
  // --- Блок 6. Эксплуатация помещения (всегда) ---
  {
    ...
    // Добавлено 05.09.2026, по вопросу клиентки: единой федеральной нормы
    // "обычного жилищного договора достаточно" или "нужен отдельный" нет —
    // решает конкретный региональный оператор ТКО (не выдаём это за
    // однозначный факт ни в одну, ни в другую сторону).
    hint: 'Если принимаете клиентов у себя дома и вывоз мусора уже покрыт обычным жилищным договором — единой федеральной нормы, нужен ли ОТДЕЛЬНЫЙ договор именно на предпринимательскую деятельность, нет. Это решает конкретный региональный оператор по обращению с ТКО — при сомнениях стоит один раз уточнить у него напрямую.',
    showIf: null,
    answers: [
      { label: 'Да', points: 1 },
      { label: 'Не уверен', points: 0.5 },
