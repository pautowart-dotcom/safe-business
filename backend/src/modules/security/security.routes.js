...
// Правила "ниша → добавить/скрыть модуль" переехали в ./nicheModules.js
// (29.08.2026) — используются и здесь (сохранение полного профиля), и на
// регистрации (auth.routes.js, signup_niche), поэтому вынесены в общее
// место, а не дублируются.
...
// Уточнено 29.08.2026: тест безопасности и просмотр результата (индекс,
// зона, карта нарушений) бесплатны для всех, скачивание PDF — платное
// (paywall внутри report.routes.js, не здесь, см. комментарий там).
// available=false здесь означает только одно: контент ни для одной
