...
const REGISTRY = [
  {
    name: 'log_visit',
    description:
      'Записать визит клиента: услуга, сумма, мастер (если есть). Выручка добавляется АВТОМАТИЧЕСКИ — не ' +
      'вызывай отдельно create_income за ту же сумму. Не для скидки, абонемента, фото до/после или списания ' +
      'материалов — для этого нужен раздел "Визиты" в приложении, честно скажи об этом пользователю.',
    parameters: {
      type: 'object',
      properties: {
        clientName: { type: 'string', description: 'Имя клиента, как назвал пользователь, дословно.' },
        service: { type: 'string', description: 'Услуга своими словами пользователя, например "маникюр с покрытием".' },
        amount: { type: 'number', description: 'Сумма визита в рублях, положительное число.' },
        masterName: {
          type: 'string',
          description: 'Имя мастера, только если пользователь явно назвал — не угадывай.',
        },
        occurredAt: {
          type: 'string',
          description: 'Дата ГГГГ-ММ-ДД, только если названа явно — иначе не указывай, будет сегодняшняя.',
        },
        paymentMethod: {
          type: 'string',
          enum: CHAT_PAYMENT_METHODS,
          description: `Способ оплаты, только если назван: ${CHAT_PAYMENT_METHODS.join(', ')}.`,
        },
      },
  ...
  },
  {
    name: 'create_expense',
    description:
      'Внести разовый переменный расход (не регулярный ежемесячный). Категория — строго одно значение из ' +
      'списка; если пользователь называет её своими словами, подбери ближайшую, а при сомнении переспроси, не угадывай.',
    parameters: {
      type: 'object',
      properties: {
        amount: {
        ...
        comment: {
          type: 'string',
          description: 'Короткое описание своими словами, например "аренда за август". Необязательно.',
        },
        channel: {
          type: 'string',
          description: 'Только если category = "advertising" — канал рекламы (Instagram, Директ, листовки и т.п.).',
        },
      },
  ...
  {
    name: 'create_income',
    description: 'Внести разовую выручку вручную (не автоматически из визита).',
    parameters: {
      type: 'object',
      properties: {
        ...
        comment: { type: 'string', description: 'Короткое описание своими словами, например "продажа сертификата". Необязательно.' },
      },
      required: ['amount'],
    },
  ...
  },
  {
    ...
    description:
      'Выручка и переменные расходы за период (только чтение). Для стандартных периодов — period. Для ' +
      '"последние N дней" — daysAgo=N вместо period, не вычисляй календарные даты сам, сервер посчитает.',
    readOnly: true,
    parameters: {
      type: 'object',
      properties: {
        period: {
          ...
          description: 'today/week/month/lastMonth (по умолчанию today). Не указывай вместе с daysAgo.',
        },
        daysAgo: {
          type: 'number',
          description: 'Число дней назад от сегодня (например 3). Если указано — period игнорируется.',
        },
      },
    },
  ...
  },
  {
    ...
    description: 'Индекс безопасности компании и список открытых нарушений теста безопасности (только чтение).',
    readOnly: true,
