...
// 05.08.2026: раздел "Журналы" (электронный учёт — УФ-лампа, инструктаж,
// дезсредства) заморожен целиком по решению владельца, до легализации
// электронных журналов. Блокировка здесь — не только в меню (More.jsx) —
// чтобы прямой вызов API (старая версия фронта в кэше, ручной запрос) не
// продолжал создавать записи, пока вопрос не решён на уровне закона.
// Полностью удалить раздел не стали — вернуть обратно должно быть так же
// просто, как убрать этот блок.
router.use((req, res) => {
  res.status(403).json({ error: 'Раздел временно недоступен' });
});
