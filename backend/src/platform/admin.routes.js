...
// 05.09.2026 — раньше ИИ составлял объяснение только по ЗАГОЛОВКУ документа
// ("О внесении изменений в статью Х..."), без самого текста закона —
// заголовки такого рода сами по себе ничего не объясняют, черновик выходил
// пустым/общим (обнаружено на реальной публикации владельцем). Теперь
// подтягиваем реальный текст со страницы pravo.gov.ru (doc_url) и отдаём его
// в промпт — грубая очистка HTML, без библиотек парсинга (страница одна и
// та же по структуре у всех документов источника, сложный парсер не нужен).
// Не проверено вживую на реальном ключе/сервере (тот же класс оговорки, что
// и у lawChangeMonitor.js — WebFetch из этой среды не достучался до
// *.gov.ru, прямой fetch с сервера должен). Если не получилось — честно
// падаем обратно на заголовок+ключевые слова, не роняем весь эндпоинт.
...
//
// "Что делать" сознательно не автоматизируем сильнее, чем "свериться со
// специалистом" — владелец прямо сказал (05.09.2026), что сам не знает
// точных налоговых/юридических шагов под конкретную ситуацию клиента, и
// придумывать их (ИИ или руками) — реальный риск навредить, а не экономия
// времени. Это осознанная граница продукта, не техническое ограничение.
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { rows } = await pool.query(
      `SELECT title, doc_url AS "docUrl", matched_keywords AS "matchedKeywords" FROM law_change_candidates WHERE id = $1`,
      [req.params.id]
    );
    const candidate = rows[0];
    if (!candidate) return res.status(404).json({ error: 'Кандидат не найден' });

    const sourceText = await fetchLawText(candidate.docUrl);
    ...
    const explanation = await draftLawExplanation({
      system:
        ...
        'законодательстве. Пиши просто и честно, без канцелярита, 4-7 предложений. Обязательно укажи: что конкретно ' +
        'меняется (по тексту документа, не только по названию), кого из бизнесов это вероятно касается (например, ' +
        '"если вы на патенте/УСН/самозанятый" — но только если это ясно из текста, не гадай), и явно порекомендуй ' +
        'свериться с бухгалтером/юристом для точных действий именно в их случае — НИКОГДА не предлагай конкретные ' +
        'шаги ("подайте заявление до...", "смените режим на...") как готовую инструкцию, это может быть неверно для ' +
        'конкретной ситуации и навредит, если ошибиться. Никогда не обещай гарантированный результат. Если текста ' +
        'документа нет и по одному заголовку с ключевыми словами не очевидно, что именно меняется — честно скажи, ' +
        'что не хватает конкретики, не придумывай последствия.',
      prompt: sourceText
        ? `Заголовок: «${candidate.title}»\nКлючевые слова: ${(candidate.matchedKeywords || []).join(', ') || 'нет'}\n\nТекст документа (может быть обрезан):\n${sourceText}`
        : `Заголовок: «${candidate.title}»\nКлючевые слова: ${(candidate.matchedKeywords || []).join(', ') || 'нет'}\n\n(Текст документа получить не удалось — работай только по заголовку и ключевым словам, честно предупреди в тексте, что конкретики может не хватать.)`,
      maxTokens: 900,
    });

    res.json({ explanation, sourceTextFetched: !!sourceText });
  })
);
