...
export function AuthProvider({ children }) {
  ...
  async function verifyCode(email, code) {
    // Снимок токена ДО запроса — см. комментарий в catch ниже про гонку
    // с конкурирующим запросом (16.09.2026, реальная жалоба владельца).
    ...
    try {
      ...
      // Гонка на уровне двух РАЗНЫХ запросов с одним и тем же (одноразовым)
      // кодом — например, автоподстановка кода почтовым клиентом на
      // телефоне почти одновременно с ручным нажатием "Подтвердить".
      // submittingRef в Login.jsx блокирует повторный сабмит В ПРЕДЕЛАХ
      // одной формы, но не спасает, если второй запрос пришёл откуда-то ещё
      // — один из двух запросов код принимает и логинит (пишет token в
      // localStorage синхронно, см. applyAuthResult), второй видит код уже
      // использованным и падает с "неверный". Раньше в этот момент
      // показывалась ошибка, хотя сессия уже реально создана — помогало
      // только обновление страницы. Проверяем: если во время ЭТОГО запроса
      // в localStorage появился новый токен, которого не было до него —
      // значит, конкурирующий запрос уже успешно вошёл, и это не настоящая
      // ошибка. Не трогаем codeAcceptedButFollowupFailed — та ветка уже сама
      // по себе означает "код принят", ей эта проверка не нужна.
