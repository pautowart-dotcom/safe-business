...
const SEGMENTS = [
  ...
  {
    key: 'auto', label: 'Услуги для автомобилей',
    niches: [
  ...
  },
  {
    ...
    niches: [
      ...
      { key: 'dog_training', label: 'Дрессировка собак' },
    ],
  },
  {
...
function SegmentationForm({ initial, onSaved, onCancel }) {
  ...
  return (
    <div>
      ...
      {/* 30.08.2026: честно предупреждаем — универсальный слой (Фаза C)
          сознательно короче полной нишевой матрицы (11 вопросов против ~34
          у, например, маникюра). Без этого человек мог бы решить, что
          "зелёная зона" — это полноценный аудит именно его отрасли. */}
      {niches.includes('universal') && (
        <div style={{ fontSize: 12.5, color: C.subtle, background: C.surface, borderRadius: 10, padding: '10px 12px', marginTop: -8, marginBottom: 16, lineHeight: 1.5 }}>
          Это базовая проверка — то, что применимо почти к любому бизнесу (регистрация, персональные данные, кассы, пожарная безопасность, кадры). Отраслевой специфики вашей сферы деятельности здесь нет — если позже появится полноценный тест именно для неё, часть требований может добавиться.
        </div>
      )}
