...
//
// 20.08.2026: раньше текст был жёстко "под студию" ("Ведите визиты клиентов,
// привязывайте мастера...") — владелец лично протестировал регистрацию
// клининговой компании и споткнулся об это. Термин роли ("Мастер"/
// "Сотрудник") теперь берётся из ui/roleLabels.js (та же логика, что уже
// применена в Users.jsx/Visits.jsx/Finance.jsx и т.д.) — единственное слово,
// которое реально меняется по нише во всём тексте инструкции. "студией"
// заменено на нейтральное "бизнесом" — не только у клининга звучало не в
// тему, часть контента (документы/сроки/советники) и так уже вычисляется по
// реальной нише отдельно, инструкции незачем притворяться, что знает больше.
// Остальной текст (визиты/абонементы/финансы/безопасность) одинаково
// применим к любой нише — переписывать его целиком под каждую из девяти
// ниш было бы избыточно ради слов, которые и так работают везде.
function buildSlides(masterLabel, masterLabelGen, masterLabelGenPlural) {
  const commonSlide = {
    icon: 'shield',
    title: 'Добро пожаловать в Безопасный бизнес',
    text: 'Сервис для управления бизнесом и снижения рисков — учёт визитов и финансов вместе с бесплатным аудитом безопасности бизнеса, чтобы штрафы и проверки не были сюрпризом.',
  };
  ...
  // "Шаблоны документов" добавлены 13.08.2026 (гейт снят). "Франшиза" и
  // "Паспорт бизнеса" по-прежнему НЕ упоминаются — ещё в тихой обкатке
  // (requireTestCompany), обещать в инструкции то, чего нет на экране, хуже,
  // чем не упомянуть вовсе.
  const clientsSlide = {
    icon: 'clients',
    title: 'Клиенты',
  ...
  return {
    owner: [
      ...
      },
      {
        ...
        text: 'Бесплатный тест безопасности бизнеса покажет индекс и конкретные нарушения с планом устранения — навсегда бесплатно, доступно только вам как владельцу.',
      },
      {
        icon: 'doc',
      ...
      },
      {
        ...
        text: 'В разделе "Безопасность" — готовые документы под вашу нишу (оферта, согласия на обработку данных и т.д.), заполненные данными вашей компании. Разовая оплата, доступ навсегда. Часть шаблонов ещё в статусе "бета" — дорабатываются, это не юридическая консультация.',
      },
      {
        icon: 'team',
        title: 'Команда',
        text: `В разделе "Ещё" — приглашение ${masterLabelGenPlural.toLowerCase()} и администраторов, дедлайны и обратная связь от команды.`,
      },
      {
        icon: 'settings',
        title: 'Подписка',
        text: 'В "Настройках" — оплата подписки на платформу и данные компании. Подробную справку по каждому разделу с примерами экранов найдёте в "Ещё" → "Как пользоваться" в любой момент.',
      },
    ],
    admin: [
      commonSlide,
      ...
      },
      {
        icon: 'team',
        title: 'Команда',
        text: `Вы можете приглашать ${masterLabelGenPlural} и следить за дедлайнами компании. Раздел "Безопасность" и настройки подписки — только у владельца. Эту инструкцию можно открыть заново через "Ещё" → "Как пользоваться", а более подробная справка с примерами экранов — там же, "Подробная справка".`,
      },
    ],
    master: [
      commonSlide,
